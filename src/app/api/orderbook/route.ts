/**
 * 호가(Orderbook) 조회 API Route (Edge Runtime)
 *
 * GET /api/orderbook?markets=KRW-BTC
 * → Upbit GET /v1/orderbook?markets=...
 *
 * 인증 불필요 (공개 시세 데이터)
  ...
  try {
    ...
    if (!markets) {
      return NextResponse.json(
        { error: 'markets 파라미터가 필요합니다 (예: KRW-BTC)' },
        { status: 400 }
      )
    }
    ...
    const response = await fetch(
      `${UPBIT_API_BASE}/v1/orderbook?markets=${encodeURIComponent(markets)}`,
      {
